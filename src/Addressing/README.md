...
`APW.Addressing` provides a small .NET domain model for postal addresses plus country-specific address validation through a validator factory.
...
Country support can be extended through additional packages.
Spain support is provided by the separate `Addressing.Spain` project in the repository rather than by this core package.
